...
func main() {
	pulumi.Run(func(ctx *pulumi.Context) error {
		...
		pubKeyFile, err := ioutil.ReadFile("../ssh/faasd.pub")
		if err != nil {
			return err
		}
		...
		network, err := networking.NewNetwork(ctx, "faasd_net", &networking.NetworkArgs{
		...
		subnet, err := networking.NewSubnet(ctx, "faasd_snet", &networking.SubnetArgs{
		...
		router, err := networking.NewRouter(ctx, "faasd_router", &networking.RouterArgs{
		...
		_, err = networking.NewRouterInterface(ctx, "faasd_ri", &networking.RouterInterfaceArgs{
			RouterId: router.ID(),
			SubnetId: subnet.ID(),
		})
		if err != nil {
		...
		secgroup, err := networking.NewSecGroup(ctx, "faasd_sg", &networking.SecGroupArgs{
		...
		_, err = networking.NewSecGroupRule(ctx, "faasd_22_sgr", &networking.SecGroupRuleArgs{
		...
		_, err = networking.NewSecGroupRule(ctx, "faasd_443_sgr", &networking.SecGroupRuleArgs{
			Direction:       pulumi.String("ingress"),
			Ethertype:       pulumi.String("IPv4"),
			Protocol:        pulumi.String("tcp"),
		...
		cloudInit, err := ioutil.ReadFile("cloud-init.yaml")
		if err != nil {
			return err
		}

		config, err := cloudinit.NewConfig(ctx, "ubuntu-config", &cloudinit.ConfigArgs{
			Gzip:         pulumi.BoolPtr(true),
			Base64Encode: pulumi.BoolPtr(true),
			Parts: cloudinit.ConfigPartArray{
				&cloudinit.ConfigPartArgs{
					Content:     pulumi.String(cloudInit),
		...
		vm, err := compute.NewInstance(ctx, "faasd_vm", &compute.InstanceArgs{
		...
		fip, err := networking.NewFloatingIp(ctx, "faasd_fip", &networking.FloatingIpArgs{
			Pool: pulumi.String(Pool),
		})
		if err != nil {
			return err
		}
		_, err = compute.NewFloatingIpAssociate(ctx, "faasd_fipa", &compute.FloatingIpAssociateArgs{
			InstanceId: vm.ID(),
		...
		ctx.Export("faasd-public", fip.Address)
		return nil
	})
}
